...
x_test=pi*X[3]+(1-pi)*X[4]

ckdtreey = spatial.cKDTree(data=y, leafsize=100)

W_y=np.zeros(n_neighbor)
C_y=np.zeros((n_neighbor,n_neighbor))


dist, nbrs_index = ckdtreey.query(y_test, k=n_neighbor+1, p=norm) 
nbrs_index = np.delete(nbrs_index, 0)
nbrs_y = y[nbrs_index]

for j in range(n_neighbor):
    for k in range(n_neighbor):
        C_y[j,k] = np.dot(y_test-nbrs_y[j],y_test-nbrs_y[k])
            

invC_y = np.linalg.inv(C_y)

sum2=0
for index_l in range(invC_y.shape[0]):
    for index_k in range(invC_y.shape[0]):
        sum2 = sum2 + invC[index_l][index_k]
...
x_rec = np.dot(X[nbrs_index].transpose(),W_y)

plt.imshow(np.reshape(x_rec,(28,28)),cmap='Greys')
#plt.imshow(np.reshape(x_test,(28,28)),cmap='Greys')
#plt.savefig('p2.eps')
